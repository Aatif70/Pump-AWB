...
import 'booklet_detail_screen.dart';
...
class _BookletListScreenState extends State<BookletListScreen> with TickerProviderStateMixin {
  ...
  Future<void> _loadBooklets() async {
    ...
    try {
      // Determine pumpId from SharedPrefs/JWT token
      _pumpId ??= await SharedPrefs.getPumpId();

      if (_pumpId == null || _pumpId!.isEmpty) {
        setState(() {
          _errorMessage = 'Petrol Pump ID not found. Please login again.';
          _isLoading = false;
        });
        developer.log('BookletListScreen: Missing petrol pump ID');
        return;
      }
                ...
                child: InkWell(
                  ...
                  onTap: () {
                    Navigator.push(
                      context,
                      MaterialPageRoute(
                        builder: (context) => BookletDetailScreen(booklet: booklet),
                      ),
                    );
                  },
                  child: Padding(
