...
class FuelDispenserRepository {
  ...
  Future<ApiResponse<List<FuelDispenser>>> getFuelDispensers() async {
    ...
    debugPrint('FUEL_DISPENSER_REPO: Fetching all fuel dispensers.');
    ...
    try {
      ...
      if (token == null) {
        ...
        debugPrint('FUEL_DISPENSER_REPO: No auth token found.');
        return ApiResponse(
          success: false,
      ...
      debugPrint('FUEL_DISPENSER_REPO: GET URL: $url');
      ...
      final response = await _apiService.get<List<FuelDispenser>>(
        ...
        token: token,
        fromJson: (json) {
          debugPrint('FUEL_DISPENSER_REPO: Parsing response data: $json');
          
          if (json is Map && json.containsKey('data') && json['data'] is List) {
            final dataList = json['data'] as List;
            debugPrint('FUEL_DISPENSER_REPO: Found ${dataList.length} fuel dispensers in response data.');
            return dataList.map((item) {
              try {
                if (item is Map<String, dynamic>) {
                  return FuelDispenser.fromJson(item);
                } else {
                  debugPrint('FUEL_DISPENSER_REPO: Skipping item because it is not a Map: $item');
                  return null;
                }
              } catch (e) {
                debugPrint('FUEL_DISPENSER_REPO: Error parsing individual dispenser: $item, Error: $e');
                return null;
              }
            }).whereType<FuelDispenser>().toList();
          } else if (json is List) {
            // Handle direct list response
            debugPrint('FUEL_DISPENSER_REPO: Response data is a direct list. Found ${json.length} items.');
            return json.map((item) {
              try {
                if (item is Map<String, dynamic>) {
                  return FuelDispenser.fromJson(item);
                } else {
                  debugPrint('FUEL_DISPENSER_REPO: Skipping item from list because it is not a Map: $item');
                  return null;
                }
              } catch (e) {
                debugPrint('FUEL_DISPENSER_REPO: Error parsing individual dispenser from list: $item, Error: $e');
                return null;
              }
            }).whereType<FuelDispenser>().toList();
          }
          ...
          debugPrint('FUEL_DISPENSER_REPO: Unexpected response format, returning empty list. Format was: ${json.runtimeType}');
      ...
      if (response.success) {
        ...
        debugPrint('FUEL_DISPENSER_REPO: Successfully retrieved ${response.data?.length ?? 0} fuel dispensers.');
      } else {
        developer.log('Failed to retrieve fuel dispensers: ${response.errorMessage}');
        debugPrint('FUEL_DISPENSER_REPO: Failed to retrieve fuel dispensers: ${response.errorMessage}');
      }
      
      return response;
    } catch (e) {
      developer.log('Exception in getFuelDispensers: $e');
      debugPrint('FUEL_DISPENSER_REPO: Exception in getFuelDispensers: $e');
      return ApiResponse(
        success: false,
  ...
  Future<ApiResponse<List<FuelDispenser>>> getFuelDispensersByPetrolPumpId(String petrolPumpId) async {
    ...
    debugPrint('FUEL_DISPENSER_REPO: Fetching fuel dispensers for petrol pump ID: $petrolPumpId');
    ...
    try {
      ...
      debugPrint('FUEL_DISPENSER_REPO: GET URL for petrol pump ID $petrolPumpId: $url');
      ...
      final response = await _apiService.get<List<FuelDispenser>>(
        ...
        token: token,
        fromJson: (json) {
          debugPrint('FUEL_DISPENSER_REPO: Parsing response data for petrol pump ID $petrolPumpId: $json');
          
          List<FuelDispenser> dispensers = [];
          
          if (json is Map && json.containsKey('data') && json['data'] is List) {
            final dataList = json['data'] as List;
            debugPrint('FUEL_DISPENSER_REPO: Found ${dataList.length} fuel dispensers for petrol pump ID $petrolPumpId');
            
            dispensers = dataList.map((item) {
              try {
                return FuelDispenser.fromJson(Map<String, dynamic>.from(item as Map));
              } catch (e) {
                debugPrint('FUEL_DISPENSER_REPO: Error parsing dispenser: $e');
                return null;
              }
            }).whereType<FuelDispenser>().toList();
          } else if (json is List) {
            debugPrint('FUEL_DISPENSER_REPO: Response is a direct list with ${json.length} items');
            
            dispensers = json.map((item) {
              try {
                return FuelDispenser.fromJson(Map<String, dynamic>.from(item as Map));
              } catch (e) {
                debugPrint('FUEL_DISPENSER_REPO: Error parsing dispenser from list: $e');
                return null;
              }
            }).whereType<FuelDispenser>().toList();
          }
      ...
      if (response.success) {
        ...
        debugPrint('FUEL_DISPENSER_REPO: Successfully retrieved ${response.data?.length ?? 0} dispensers for petrol pump ID $petrolPumpId');
      } else {
        developer.log('Failed to retrieve fuel dispensers for petrol pump ID $petrolPumpId: ${response.errorMessage}');
        debugPrint('FUEL_DISPENSER_REPO: Failed to retrieve dispensers for petrol pump ID $petrolPumpId: ${response.errorMessage}');
      }
      
      return response;
    } catch (e) {
      developer.log('Exception in getFuelDispensersByPetrolPumpId: $e');
      debugPrint('FUEL_DISPENSER_REPO: Exception in getFuelDispensersByPetrolPumpId: $e');
      return ApiResponse(
        success: false,
  ...
  Future<String?> getPetrolPumpId() async {
    debugPrint('FUEL_DISPENSER_REPO: Attempting to get Petrol Pump ID.');
    try {
      // Get the authentication token
      final prefs = await SharedPreferences.getInstance();
      final token = prefs.getString(ApiConstants.authTokenKey);
      ...
      if (token == null) {
        ...
        debugPrint('FUEL_DISPENSER_REPO: No auth token found for getPetrolPumpId.');
        return null;
      }
      ...
      if (petrolPumpId == null || petrolPumpId.isEmpty) {
        ...
        debugPrint('FUEL_DISPENSER_REPO: PetrolPumpId not found in JWT. Checking prefs...');
        // Fallback to a stored ID if available
        final storedId = prefs.getString('petrolPumpId');
        debugPrint('FUEL_DISPENSER_REPO: Found stored PetrolPumpId: $storedId');
        return storedId;
      }
      
      developer.log('PetrolPumpId extracted from JWT token: $petrolPumpId');
      debugPrint('FUEL_DISPENSER_REPO: PetrolPumpId from JWT: $petrolPumpId');
      
      // Store for later use if extracted from JWT
      await prefs.setString('petrolPumpId', petrolPumpId);
      debugPrint('FUEL_DISPENSER_REPO: Stored PetrolPumpId in prefs.');
      
      return petrolPumpId;
    } catch (e, stacktrace) {
      developer.log('Error getting petrol pump ID: $e');
      debugPrint('FUEL_DISPENSER_REPO: Error in getPetrolPumpId: $e');
      debugPrint('FUEL_DISPENSER_REPO: Stacktrace: $stacktrace');
      return null;
    }
  }
  ...
  Future<ApiResponse<FuelDispenser>> addFuelDispenser(FuelDispenser dispenser) async {
    ...
    debugPrint('FUEL_DISPENSER_REPO: Adding fuel dispenser: ${dispenser.toJson()}');
    ...
    try {
      ...
      final url = ApiConstants.getFuelDispenserUrl();
      debugPrint('FUEL_DISPENSER_REPO: POST URL: $url');
      
      // Create payload according to the API spec
      final payload = <String, dynamic>{
        'dispenserNumber': dispenser.dispenserNumber,
        'numberOfNozzles': dispenser.numberOfNozzles,
        'status': dispenser.status,
      };
      if (dispenser.dispenserName != null && dispenser.dispenserName!.isNotEmpty) {
        payload['dispenserName'] = dispenser.dispenserName;
      }
      
      debugPrint('FUEL_DISPENSER_REPO: Request body: ${json.encode(payload)}');
      ...
      final response = await _apiService.post<FuelDispenser>(
        ...
        token: token,
        fromJson: (json) {
          debugPrint('FUEL_DISPENSER_REPO: Response from add dispenser: $json');
      ...
      if (response.success) {
        ...
        debugPrint('FUEL_DISPENSER_REPO: Fuel dispenser added successfully. Response: ${response.data}');
      } else {
        developer.log('Failed to add fuel dispenser: ${response.errorMessage}');
        debugPrint('FUEL_DISPENSER_REPO: Failed to add fuel dispenser: ${response.errorMessage}');
      }
      
      return response;
    } catch (e) {
      developer.log('Exception in addFuelDispenser: $e');
      debugPrint('FUEL_DISPENSER_REPO: Exception in addFuelDispenser: $e');
      return ApiResponse(
        success: false,
  ...
  Future<ApiResponse<FuelDispenser>> getFuelDispenserById(String id) async {
    ...
    debugPrint('FUEL_DISPENSER_REPO: Fetching fuel dispenser with ID: $id');
    ...
    try {
      ...
      debugPrint('FUEL_DISPENSER_REPO: GET URL for ID $id: $url');
      ...
      final response = await _apiService.get<FuelDispenser>(
        ...
        token: token,
        fromJson: (json) {
          debugPrint('FUEL_DISPENSER_REPO: Response for dispenser ID $id: $json');
          ...
          debugPrint('FUEL_DISPENSER_REPO: Unexpected response format for dispenser ID $id');
      ...
      if (response.success) {
        ...
        debugPrint('FUEL_DISPENSER_REPO: Fuel dispenser retrieved successfully.');
      } else {
        developer.log('Failed to get fuel dispenser: ${response.errorMessage}');
        debugPrint('FUEL_DISPENSER_REPO: Failed to get fuel dispenser: ${response.errorMessage}');
      }
      
      return response;
    } catch (e) {
      developer.log('Exception in getFuelDispenserById: $e');
      debugPrint('FUEL_DISPENSER_REPO: Exception in getFuelDispenserById: $e');
      return ApiResponse(
        success: false,
        errorMessage: e.toString(),
      );
  ...
  Future<ApiResponse<FuelDispenser>> updateFuelDispenser(FuelDispenser dispenser) async {
    ...
    try {
      ...
      if (validNozzleCount < 1) {
        debugPrint('FUEL_DISPENSER_REPO: Correcting invalid nozzle count (${dispenser.numberOfNozzles}) to 1');
        validNozzleCount = 1;
      } else if (validNozzleCount > 6) {
        debugPrint('FUEL_DISPENSER_REPO: Correcting invalid nozzle count (${dispenser.numberOfNozzles}) to 6');
        validNozzleCount = 6;
      }
      
      // Create payload for updates
      final payload = <String, dynamic>{
        'dispenserNumber': dispenser.dispenserNumber,
      ...
      debugPrint('FUEL_DISPENSER_REPO: Exception in updateFuelDispenser: $e');
      return ApiResponse(
        success: false,
        errorMessage: e.toString(),
      );
  ...
  Future<ApiResponse<bool>> deleteFuelDispenser(String id) async {
    ...
    debugPrint('FUEL_DISPENSER_REPO: Deleting fuel dispenser ID: $id');
    ...
    try {
      ...
      final url = ApiConstants.getFuelDispenserByIdUrl(id);
      debugPrint('FUEL_DISPENSER_REPO: DELETE URL: $url');
      ...
      if (response.success) {
        ...
        debugPrint('FUEL_DISPENSER_REPO: Fuel dispenser deleted successfully.');
      } else {
        developer.log('Failed to delete fuel dispenser: ${response.errorMessage}');
        debugPrint('FUEL_DISPENSER_REPO: Failed to delete fuel dispenser: ${response.errorMessage}');
      }
      
      return response;
    } catch (e) {
      developer.log('Exception in deleteFuelDispenser: $e');
      debugPrint('FUEL_DISPENSER_REPO: Exception in deleteFuelDispenser: $e');
      return ApiResponse(
        success: false,
        errorMessage: e.toString(),
      );
