...
class FuelDeliveryRepository {
  ...
  Future<ApiResponse<Map<String, dynamic>>> addFuelDelivery(FuelDelivery fuelDelivery) async {
    ...
    debugPrint('FUEL_DELIVERY_REPO: Adding fuel delivery: ${fuelDelivery.toJson()}');
    debugPrint('REQUEST PAYLOAD:');
    debugPrint('---------------');
    debugPrint('deliveryDate: ${fuelDelivery.deliveryDate.toIso8601String()}');
    debugPrint('fuelTankId: ${fuelDelivery.fuelTankId}');
    debugPrint('invoiceNumber: ${fuelDelivery.invoiceNumber}');
    debugPrint('quantityReceived: ${fuelDelivery.quantityReceived}');
    debugPrint('supplierId: ${fuelDelivery.supplierId}');
    debugPrint('density: ${fuelDelivery.density}');
    debugPrint('temperature: ${fuelDelivery.temperature}');
    debugPrint('notes: ${fuelDelivery.notes ?? "EMPTY"}');
    debugPrint('---------------');
    ...
    try {
      ...
      if (token == null) {
        ...
        debugPrint('FUEL_DELIVERY_REPO: No auth token found.');
        return ApiResponse(
          success: false,
      ...
      debugPrint('FUEL_DELIVERY_REPO: POST URL: $url');
      ...
      debugPrint('Error Message: ${response.errorMessage ?? "NONE"}');
      debugPrint('---------------');
      
      if (response.success) {
        developer.log('Fuel delivery added successfully');
        debugPrint('FUEL_DELIVERY_REPO: Fuel delivery added successfully. Response: ${response.data}');
      } else {
        developer.log('Failed to add fuel delivery: ${response.errorMessage}');
        debugPrint('FUEL_DELIVERY_REPO: Failed to add fuel delivery: ${response.errorMessage}');
      }
      
      return response;
    } catch (e) {
      developer.log('Exception in addFuelDelivery: $e');
      debugPrint('FUEL_DELIVERY_REPO: Exception in addFuelDelivery: $e');
      return ApiResponse(
        success: false,
  ...
  Future<ApiResponse<List<FuelDelivery>>> getAllFuelDeliveries() async {
    ...
    debugPrint('FUEL_DELIVERY_REPO: Fetching all fuel deliveries.');
    ...
    try {
      ...
      debugPrint('FUEL_DELIVERY_REPO: GET URL: $url');
      ...
      final response = await _apiService.get<List<FuelDelivery>>(
        ...
        token: token,
        fromJson: (json) {
          debugPrint('FUEL_DELIVERY_REPO: Parsing response data: $json');
          // Expecting the structure: {"data": [...], "success": true, ...}
          if (json is Map && json.containsKey('data') && json['data'] is List) {
            final dataList = json['data'] as List;
            debugPrint('FUEL_DELIVERY_REPO: Found ${dataList.length} fuel deliveries in response data.');
            return dataList.map((item) {
              try {
                ...
                if (item is Map<String, dynamic>) {
                  ...
                  debugPrint('FUEL_DELIVERY_REPO: Skipping item because it is not a Map: $item');
                  return null; // Skip non-map items
                }
              } catch (e, stacktrace) {
                 debugPrint('FUEL_DELIVERY_REPO: Error parsing individual fuel delivery: $item, Error: $e');
                 debugPrint('FUEL_DELIVERY_REPO: Stacktrace: $stacktrace');
             ...
             debugPrint('FUEL_DELIVERY_REPO: Response data is a direct list. Found ${json.length} items.');
             return json.map((item) {
               try {
                 if (item is Map<String, dynamic>) {
                    return FuelDelivery.fromJson(item);
                 } else {
                   debugPrint('FUEL_DELIVERY_REPO: Skipping item from list because it is not a Map: $item');
                   return null;
                 }
               } catch (e, stacktrace) {
                 debugPrint('FUEL_DELIVERY_REPO: Error parsing individual fuel delivery from list: $item, Error: $e');
                 debugPrint('FUEL_DELIVERY_REPO: Stacktrace: $stacktrace');
                 return null;
               }
          ...
          debugPrint('FUEL_DELIVERY_REPO: Unexpected response format, returning empty list. Format was: ${json.runtimeType}');
          return <FuelDelivery>[];
        },
      );
      ...
      if (response.success) {
        ...
        debugPrint('FUEL_DELIVERY_REPO: Successfully retrieved ${response.data?.length ?? 0} fuel deliveries.');
      } else {
        developer.log('Failed to retrieve fuel deliveries: ${response.errorMessage}');
        debugPrint('FUEL_DELIVERY_REPO: Failed to retrieve fuel deliveries: ${response.errorMessage}');
      }
      
      return response;
    } catch (e, stacktrace) {
      developer.log('Exception in getAllFuelDeliveries: $e');
      debugPrint('FUEL_DELIVERY_REPO: Exception in getAllFuelDeliveries: $e');
      debugPrint('FUEL_DELIVERY_REPO: Stacktrace: $stacktrace');
      return ApiResponse(
        success: false,
  ...
  Future<ApiResponse<List<FuelDeliveryOrder>>> getAllFuelDeliveryOrders() async {
    ...
    debugPrint('FUEL_DELIVERY_REPO: Fetching all fuel delivery orders.');
    ...
    try {
      ...
      if (petrolPumpId == null) {
        ...
        debugPrint('FUEL_DELIVERY_REPO: Petrol pump ID not found.');
        return ApiResponse(
          success: false,
      ...
      final response = await _apiService.get<List<FuelDeliveryOrder>>(
        ...
        fromJson: (json) {
          ...
          if (json is List) {
            debugPrint('FUEL_DELIVERY_REPO: Processing direct list response with ${json.length} items');
            return json.map((item) {
              try {
                if (item is Map<String, dynamic>) {
                ...
                debugPrint('FUEL_DELIVERY_REPO: Error parsing individual fuel delivery order: $item, Error: $e');
                debugPrint('FUEL_DELIVERY_REPO: Stacktrace: $stacktrace');
                return null;
              }
          ...
          // Handle wrapped response format with data field
            ...
            final dataList = json['data'] as List;
            debugPrint('FUEL_DELIVERY_REPO: Found ${dataList.length} fuel delivery orders in wrapped response data.');
            return dataList.map((item) {
              try {
                if (item is Map<String, dynamic>) {
                  return FuelDeliveryOrder.fromJson(item);
      ...
      if (response.success) {
        ...
        debugPrint('FUEL_DELIVERY_REPO: Successfully retrieved ${response.data?.length ?? 0} fuel delivery orders.');
      } else {
        developer.log('Failed to retrieve fuel delivery orders: ${response.errorMessage}');
        debugPrint('FUEL_DELIVERY_REPO: Failed to retrieve fuel delivery orders: ${response.errorMessage}');
      }
      
      return response;
    } catch (e, stacktrace) {
      developer.log('Exception in getAllFuelDeliveryOrders: $e');
      debugPrint('FUEL_DELIVERY_REPO: Exception in getAllFuelDeliveryOrders: $e');
      debugPrint('FUEL_DELIVERY_REPO: Stacktrace: $stacktrace');
      return ApiResponse(
        success: false,
  ...
  Future<String?> getPetrolPumpId() async {
    debugPrint('FUEL_DELIVERY_REPO: Attempting to get Petrol Pump ID.');
    try {
      // Get the authentication token
      final prefs = await SharedPreferences.getInstance();
      final token = prefs.getString(ApiConstants.authTokenKey);
      ...
      if (token == null) {
        ...
        debugPrint('FUEL_DELIVERY_REPO: No auth token found for getPetrolPumpId.');
        return null;
      }
      ...
      if (petrolPumpId == null || petrolPumpId.isEmpty) {
        ...
        debugPrint('FUEL_DELIVERY_REPO: PetrolPumpId not found in JWT. Checking prefs...');
        // Fallback to a stored ID if available
        final storedId = prefs.getString('petrolPumpId');
        debugPrint('FUEL_DELIVERY_REPO: Found stored PetrolPumpId: $storedId');
        return storedId;
      }
      
      developer.log('PetrolPumpId extracted from JWT token: $petrolPumpId');
      debugPrint('FUEL_DELIVERY_REPO: PetrolPumpId from JWT: $petrolPumpId');
      
      // Store for later use if extracted from JWT
      await prefs.setString('petrolPumpId', petrolPumpId);
      debugPrint('FUEL_DELIVERY_REPO: Stored PetrolPumpId in prefs.');
      
      return petrolPumpId;
    } catch (e, stacktrace) {
      developer.log('Error getting petrol pump ID: $e');
      debugPrint('FUEL_DELIVERY_REPO: Error in getPetrolPumpId: $e');
      debugPrint('FUEL_DELIVERY_REPO: Stacktrace: $stacktrace');
      return null;
    }
  }
