...
class PumpRepository {
  ...
  Future<ApiResponse<PumpProfile>> getPumpProfile() async {
    ...
    debugPrint('PUMP_REPO: Fetching pump profile.');
    ...
    try {
      ...
      if (token == null) {
        ...
        debugPrint('PUMP_REPO: No auth token found.');
        return ApiResponse(
          success: false,
      ...
      if (petrolPumpId == null || petrolPumpId.isEmpty) {
        ...
        debugPrint('PUMP_REPO: PetrolPumpId not found in JWT token');
        return ApiResponse(
          success: false,
      ...
      debugPrint('PUMP_REPO: GET URL: $url');
      ...
      final response = await _apiService.get<PumpProfile>(
        ...
        fromJson: (dynamic json) {
          debugPrint('PUMP_REPO: Raw response data: $json');
          
          // First check if we have a data field
          if (json is Map && json.containsKey('data')) {
            final dataJson = json['data'];
            if (dataJson is Map<String, dynamic>) {
              debugPrint('PUMP_REPO: Found data object, parsing PumpProfile');
          ...
          if (json is Map<String, dynamic>) {
            debugPrint('PUMP_REPO: Direct object format, parsing PumpProfile');
            return PumpProfile.fromJson(json);
          }
          
          // If response format is unexpected
          debugPrint('PUMP_REPO: Unexpected response format: ${json.runtimeType}');
      ...
      if (response.success) {
        ...
        debugPrint('PUMP_REPO: Successfully retrieved pump profile: ${response.data?.toJson()}');
      } else {
        developer.log('Failed to retrieve pump profile: ${response.errorMessage}');
        debugPrint('PUMP_REPO: Failed to retrieve pump profile: ${response.errorMessage}');
      }
      
      return response;
    } catch (e) {
      developer.log('Exception in getPumpProfile: $e');
      debugPrint('PUMP_REPO: Exception in getPumpProfile: $e');
      return ApiResponse(
        success: false,
  ...
  Future<ApiResponse<bool>> updatePumpProfile(PumpProfile profile) async {
    ...
    debugPrint('PUMP_REPO: Updating pump profile.');
    ...
    try {
      ...
      if (profile.petrolPumpId == null || profile.petrolPumpId!.isEmpty) {
        ...
        debugPrint('PUMP_REPO: PetrolPumpId not provided for update');
        return ApiResponse(
          success: false,
      ...
      if (profile.fuelTypesAvailable.split(',').length > 3) {
        debugPrint('PUMP_REPO: Warning - more than 3 fuel types selected, API may reject.');
      }
      
      final url = ApiConstants.getUpdatePumpProfileUrl(profile.petrolPumpId!);
      developer.log('Update Pump Profile URL: $url');
      debugPrint('PUMP_REPO: PUT URL: $url');
      
      // Convert the profile to JSON
      final profileJson = profile.toJson();
      debugPrint('PUMP_REPO: Profile data for update: $profileJson');
      ...
      final response = await _apiService.put<bool>(
        ...
        token: token,
        fromJson: (dynamic json) {
          debugPrint('PUMP_REPO: Update response: $json');
      ...
      if (response.success) {
        ...
        debugPrint('PUMP_REPO: Successfully updated pump profile');
      } else {
        developer.log('Failed to update pump profile: ${response.errorMessage}');
        debugPrint('PUMP_REPO: Failed to update pump profile: ${response.errorMessage}');
        
        // If there's an issue with fuel types, try fallback approach
        if (!response.success && 
            profile.fuelTypesAvailable.split(',').length > 1) {
          debugPrint('PUMP_REPO: Initial update failed. Trying with just the primary fuel type.');
          ...
          debugPrint('PUMP_REPO: Fallback profile data: $fallbackJson');
          ...
          if (fallbackResponse.success) {
            debugPrint('PUMP_REPO: Fallback approach with single fuel type successful');
            return fallbackResponse;
          } else {
            debugPrint('PUMP_REPO: Fallback approach also failed: ${fallbackResponse.errorMessage}');
      ...
      debugPrint('PUMP_REPO: Exception in updatePumpProfile: $e');
      return ApiResponse(
        success: false,
  ...
  Future<ApiResponse<List<FuelType>>> getFuelTypes() async {
    ...
    debugPrint('PUMP_REPO: Fetching fuel types.');
    ...
    try {
      ...
      debugPrint('PUMP_REPO: GET URL for fuel types: $url');
      ...
      final response = await _apiService.get<List<FuelType>>(
        ...
        token: token,
        fromJson: (dynamic json) {
          debugPrint('PUMP_REPO: Raw fuel types data: $json');
          ...
          if (json is Map && json.containsKey('data') && json['data'] is List) {
            ...
            debugPrint('PUMP_REPO: Found ${dataList.length} fuel types in data list');
          ...
          // If the response is directly a list
            ...
            debugPrint('PUMP_REPO: Found ${json.length} fuel types in direct list');
          ...
          debugPrint('PUMP_REPO: Parsed ${fuelTypes.length} fuel types');
          return fuelTypes;
        },
      );
      ...
      if (response.success) {
        ...
        debugPrint('PUMP_REPO: Successfully retrieved ${response.data?.length ?? 0} fuel types');
      } else {
        developer.log('Failed to retrieve fuel types: ${response.errorMessage}');
        debugPrint('PUMP_REPO: Failed to retrieve fuel types: ${response.errorMessage}');
      }
      
      return response;
    } catch (e) {
      developer.log('Exception in getFuelTypes: $e');
      debugPrint('PUMP_REPO: Exception in getFuelTypes: $e');
      return ApiResponse(
        success: false,
  ...
  Future<ApiResponse<List<FuelType>>> getPumpFuelTypes(String petrolPumpId) async {
    ...
    debugPrint('PUMP_REPO: Fetching fuel types for petrol pump: $petrolPumpId');
    ...
    try {
      ...
      debugPrint('PUMP_REPO: GET URL for pump fuel types: $url');
      ...
      final response = await _apiService.get<List<FuelType>>(
        ...
        token: token,
        fromJson: (dynamic json) {
          debugPrint('PUMP_REPO: Raw pump fuel types data: $json');
          ...
          if (json is Map && json.containsKey('data') && json['data'] is List) {
            final dataList = json['data'] as List;
            debugPrint('PUMP_REPO: Found ${dataList.length} pump fuel types in data list');
          ...
          // If the response is directly a list
            ...
            debugPrint('PUMP_REPO: Found ${json.length} pump fuel types in direct list');
          ...
          debugPrint('PUMP_REPO: Parsed ${fuelTypes.length} pump fuel types');
          return fuelTypes;
        },
      );
      ...
      if (response.success) {
        ...
        debugPrint('PUMP_REPO: Successfully retrieved ${response.data?.length ?? 0} pump fuel types');
      } else {
        developer.log('Failed to retrieve pump fuel types: ${response.errorMessage}');
        debugPrint('PUMP_REPO: Failed to retrieve pump fuel types: ${response.errorMessage}');
      }
      
      return response;
    } catch (e) {
      developer.log('Exception in getPumpFuelTypes: $e');
      debugPrint('PUMP_REPO: Exception in getPumpFuelTypes: $e');
      return ApiResponse(
        success: false,
