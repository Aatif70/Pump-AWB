...
class EmployeeShiftRepository {
  ...
  Future<ApiResponse<bool>> deleteEmployeeShift(String employeeShiftId) async {
    debugPrint('REPOSITORY: Deleting employee shift with ID: $employeeShiftId');
    ...
    try {
      ...
      if (token == null) {
        debugPrint('REPOSITORY: No auth token found for deleting employee shift');
        return ApiResponse(
          success: false,
          errorMessage: 'You are not logged in. Please login to continue.',
        );
      ...
      debugPrint('REPOSITORY: DELETE URL: $url');
      ...
      // Log response details
      debugPrint('REPOSITORY: DELETE_RESPONSE: ${response.success ? 'Success' : 'Failed'} - ${response.errorMessage ?? ''}');
      
      if (response.success) {
        debugPrint('REPOSITORY: Employee shift deleted successfully');
      }
      
      return response;
    } catch (e) {
      debugPrint('REPOSITORY: Exception in deleteEmployeeShift: $e');
      return ApiResponse(
        success: false,
        errorMessage: e.toString(),
      );
  ...
  Future<ApiResponse<String>> getEmployeeShiftId(String employeeId, String shiftId) async {
    ...
    try {
      ...
      debugPrint('REPOSITORY: Getting employee shifts URL: $url');
      
      final response = await _apiService.get<List<Map<String, dynamic>>>(
        url,
        token: token,
        fromJson: (json) {
          debugPrint('REPOSITORY: Employee shifts response: $json');
          ...
          if (json is Map && json.containsKey('data') && json['data'] is List) {
            ...
            debugPrint('REPOSITORY: Found ${shiftList.length} employee shifts in the data array');
          } else if (json is List) {
            shiftList = json;
            debugPrint('REPOSITORY: Found ${shiftList.length} employee shifts in direct list');
          } else {
            debugPrint('REPOSITORY: Unexpected response format: $json');
      ...
      if (!response.success || response.data == null) {
        debugPrint('REPOSITORY: Failed to get employee shifts: ${response.errorMessage}');
        return ApiResponse(
          success: false,
      ...
      debugPrint('REPOSITORY: Processing ${shifts.length} employee shifts to find match');
      
      // Debug - debugPrint all shifts
      for (var shift in shifts) {
        debugPrint('REPOSITORY: Shift record - employeeId: ${shift['employeeId']}, shiftId: ${shift['shiftId']}, employeeShiftId: ${shift['employeeShiftId']}');
      }
      ...
      if (employeeShift.isEmpty || employeeShift['employeeShiftId'] == null) {
        debugPrint('REPOSITORY: Employee shift not found for employee $employeeId and shift $shiftId');
        return ApiResponse(
          success: false,
      ...
      debugPrint('REPOSITORY: Found employeeShiftId: ${employeeShift['employeeShiftId']}');
      return ApiResponse(
        success: true,
        data: employeeShift['employeeShiftId'].toString(),
      );
    } catch (e) {
      debugPrint('REPOSITORY: Exception in getEmployeeShiftId: $e');
      return ApiResponse(
        success: false,
        errorMessage: e.toString(),
      );
  ...
  Future<ApiResponse<bool>> removeEmployeeFromShift(String employeeId, String shiftId) async {
    debugPrint('REPOSITORY: Removing employee $employeeId from shift $shiftId');
    
    try {
      // First, get the employee shift ID
      debugPrint('REPOSITORY: Getting employee shift ID for employee $employeeId in shift $shiftId');
      final idResponse = await getEmployeeShiftId(employeeId, shiftId);
      
      if (!idResponse.success || idResponse.data == null) {
        debugPrint('REPOSITORY: Failed to get employee shift ID: ${idResponse.errorMessage}');
        return ApiResponse(
          success: false,
      ...
      debugPrint('REPOSITORY: Found employee shift ID: $employeeShiftId - Proceeding with deletion');
      
      // Delete the employee shift
      return await deleteEmployeeShift(employeeShiftId);
    } catch (e) {
      debugPrint('REPOSITORY: Exception in removeEmployeeFromShift: $e');
      return ApiResponse(
        success: false,
        errorMessage: e.toString(),
      );
  ...
  Future<ApiResponse<List<Shift>>> getShiftsByEmployeeId(String employeeId) async {
    debugPrint('DEBUG_REPO: Getting shifts for employee ID: $employeeId');
    ...
    try {
      ...
      // Check if token exists
      if (token == null) {
        debugPrint('DEBUG_REPO: No auth token found for getting employee shifts');
        return ApiResponse(
          success: false,
          errorMessage: 'You are not logged in. Please login to continue.',
        );
      ...
      debugPrint('DEBUG_REPO: Requesting shifts from URL: $url');
      
      final response = await _apiService.get<List<Shift>>(
        url,
        token: token,
        fromJson: (json) {
          debugPrint('DEBUG_REPO: Raw response from API: $json');
          ...
          if (json is Map && json.containsKey('data') && json['data'] is List) {
            ...
            debugPrint('DEBUG_REPO: Found employee shifts in data field, count: ${employeeShiftList.length}');
          } else if (json is List) {
            employeeShiftList = json;
            debugPrint('DEBUG_REPO: Got direct list of employee shifts, count: ${employeeShiftList.length}');
          } else {
            debugPrint('DEBUG_REPO: Unexpected response format for employee shifts: $json');
            return <Shift>[];
          }
          ...
          try {
            for (var employeeShift in employeeShiftList) {
              debugPrint('DEBUG_REPO: Processing employee shift: $employeeShift');
              ...
              if (employeeShift is Map && employeeShift.containsKey('shift')) {
                ...
                debugPrint('DEBUG_REPO: Found shift data in employee shift: $shiftData');
                
                if (shiftData != null) {
                  try {
                    shifts.add(Shift.fromJson(shiftData));
                  } catch (e) {
                    debugPrint('DEBUG_REPO: Error parsing shift from employee shift: $e');
                  }
                }
              }
              // If shift ID is available but not the full shift object
                ...
                debugPrint('DEBUG_REPO: Found shiftId $shiftId, need to fetch shift details');
            ...
            debugPrint('DEBUG_REPO: Successfully processed ${shifts.length} shifts for employee');
            return shifts;
          } catch (e) {
            debugPrint('DEBUG_REPO: Error processing employee shifts: $e');
            return <Shift>[];
          }
        },
      );
      ...
      if (response.success) {
        final count = response.data?.length ?? 0;
        debugPrint('DEBUG_REPO: Successfully got $count shifts for employee');
      } else {
        debugPrint('DEBUG_REPO: Failed to get shifts for employee: ${response.errorMessage}');
      }
      
      return response;
    } catch (e) {
      debugPrint('DEBUG_REPO: Exception in getShiftsByEmployeeId: $e');
      return ApiResponse(
        success: false,
        errorMessage: e.toString(),
      );
  ...
  Future<ApiResponse<bool>> removeAllEmployeesFromShift(String shiftId) async {
    debugPrint('REPOSITORY: Removing all employees from shift $shiftId');
    ...
    try {
      ...
      if (token == null) {
        debugPrint('REPOSITORY: No auth token found for removing employees from shift');
        return ApiResponse(
          success: false,
          errorMessage: 'You are not logged in. Please login to continue.',
        );
      ...
      debugPrint('REPOSITORY: Getting all employees for shift $shiftId');
      final employeesResponse = await getEmployeesByShiftId(shiftId);
      
      if (!employeesResponse.success) {
        debugPrint('REPOSITORY: Failed to get employees for shift: ${employeesResponse.errorMessage}');
        return ApiResponse(
          success: false,
      ...
      debugPrint('REPOSITORY: Found ${employees.length} employees to remove from shift');
      
      if (employees.isEmpty) {
        // No employees to remove, return success
        debugPrint('REPOSITORY: No employees found for shift $shiftId');
        return ApiResponse(
          success: true,
          data: true,
        );
      ...
      for (final employee in employees) {
        if (employee.id == null || employee.id!.isEmpty) {
          debugPrint('REPOSITORY: Skipping employee with missing ID');
          continue;
        }
        
        debugPrint('REPOSITORY: Removing employee ${employee.id} (${employee.firstName} ${employee.lastName}) from shift');
        final removeResponse = await removeEmployeeFromShift(employee.id!, shiftId);
        
        if (!removeResponse.success) {
          debugPrint('REPOSITORY: Failed to remove employee ${employee.id}: ${removeResponse.errorMessage}');
          allSucceeded = false;
      ...
      if (allSucceeded) {
        debugPrint('REPOSITORY: Successfully removed all employees from shift $shiftId');
        return ApiResponse(
          success: true,
          data: true,
        );
      } else {
        debugPrint('REPOSITORY: Failed to remove some employees from shift');
        return ApiResponse(
          success: false,
      ...
      debugPrint('REPOSITORY: Exception in removeAllEmployeesFromShift: $e');
      return ApiResponse(
        success: false,
        errorMessage: e.toString(),
      );
