...
class ShiftRepository {
  ...
  Future<ApiResponse<Map<String, dynamic>>> addShift(Shift shift) async {
    ...
    debugPrint('SHIFT_REPO: Starting shift creation with data: ${shift.toJson()}');
    ...
    try {
      ...
      if (token == null) {
        ...
        debugPrint('SHIFT_REPO: Authentication failed - no token found');
        return ApiResponse(
          success: false,
      ...
      debugPrint('SHIFT_REPO: Using correct URL: $url');
      
      // Log the full request details
      // debugPrint('SHIFT_REPO: Request Token Present: ${token != null}');
      debugPrint('SHIFT_REPO: Request Body: ${shift.toJson()}');
      ...
      if (response.success) {
        ...
        debugPrint('SHIFT_REPO: Shift added successfully! Response data: ${response.data}');
        ...
        if (responseData != null) {
          ...
          if (shiftId != null) {
            debugPrint('SHIFT_REPO: Received shift ID from server: $shiftId');
          } else {
            debugPrint('SHIFT_REPO: WARNING: No shift ID found in the response');
          }
        }
      } else {
        developer.log('Failed to add shift: ${response.errorMessage}');
        debugPrint('SHIFT_REPO: Failed to add shift: ${response.errorMessage}');
      }
      
      return response;
    } catch (e) {
      developer.log('Exception in addShift: $e');
      debugPrint('SHIFT_REPO: Exception in addShift: $e');
      return ApiResponse(
        success: false,
  ...
  Future<ApiResponse<List<Shift>>> getAllShifts() async {
    ...
    debugPrint('SHIFT_REPO: Fetching all shifts');
    ...
    try {
      ...
      if (petrolPumpId == null) {
        ...
        debugPrint('SHIFT_REPO: Failed to extract petrol pump ID from token');
        return ApiResponse(
          success: false,
      ...
      debugPrint('SHIFT_REPO: Getting shifts for petrol pump ID: $petrolPumpId');
      
      // Make the API call
      final url = '${ApiConstants.baseUrl}/api/Shift/$petrolPumpId/shifts';
      developer.log('Shifts URL: $url');
      debugPrint('SHIFT_REPO: Using URL for fetching shifts: $url');
      
      final response = await _apiService.get<List<Shift>>(
        url,
        token: token,
        fromJson: (json) {
          debugPrint('SHIFT_REPO: Parsing response data: $json');
          List<Shift> shifts = [];
          
          if (json is List) {
            debugPrint('SHIFT_REPO: Response is a direct list with ${json.length} items');
            ...
            if (typedJson.containsKey('data') && typedJson['data'] is List) {
              ...
              debugPrint('SHIFT_REPO: Found ${dataList.length} shifts in response data array');
            ...
            // Handle case where the entire response is a single shift object
              ...
              debugPrint('SHIFT_REPO: Response contains a single shift object');
          ...
          for (var i = 0; i < shifts.length; i++) {
            debugPrint('SHIFT_REPO: Shift $i ID: ${shifts[i].id}');
          }
      ...
      if (response.success) {
        ...
        debugPrint('SHIFT_REPO: Successfully retrieved ${response.data?.length ?? 0} shifts');
        
        if (response.data != null && response.data!.isNotEmpty) {
          // Verify if any shifts are missing IDs
          int missingIdCount = response.data!.where((shift) => shift.id == null).length;
          if (missingIdCount > 0) {
            debugPrint('SHIFT_REPO: WARNING: $missingIdCount shifts are missing IDs');
          }
        }
      } else {
        developer.log('Failed to retrieve shifts: ${response.errorMessage}');
        debugPrint('SHIFT_REPO: Failed to retrieve shifts: ${response.errorMessage}');
      }
      
      return response;
    } catch (e) {
      developer.log('Exception in getAllShifts: $e');
      debugPrint('SHIFT_REPO: Exception in getAllShifts: $e');
      return ApiResponse(
        success: false,
        errorMessage: e.toString(),
      );
  ...
  Future<ApiResponse<Map<String, dynamic>>> updateShift(String shiftId, Shift shift) async {
    ...
    debugPrint('SHIFT_REPO: Starting shift update with ID: $shiftId and data: ${shift.toJson()}');
    ...
    try {
      ...
      if (shiftId.isEmpty) {
        ...
        debugPrint('SHIFT_REPO: Cannot update shift: Empty shift ID');
        return ApiResponse(
          success: false,
      ...
      debugPrint('SHIFT_REPO: Using URL for update: $url');
      ...
      debugPrint('SHIFT_REPO: Update request body: $requestBody');
      ...
      final response = await _apiService.put<Map<String, dynamic>>(
        ...
        token: token,
        fromJson: (json) {
          debugPrint('SHIFT_REPO: Update response: $json');
          // Ensure proper typing for the response
          if (json is Map) {
            return Map<String, dynamic>.from(json);
          }
      ...
      if (response.success) {
        ...
        debugPrint('SHIFT_REPO: Shift updated successfully! Response data: ${response.data}');
      } else {
        developer.log('Failed to update shift: ${response.errorMessage}');
        debugPrint('SHIFT_REPO: Failed to update shift: ${response.errorMessage}');
      }
      
      return response;
    } catch (e) {
      developer.log('Exception in updateShift: $e');
      debugPrint('SHIFT_REPO: Exception in updateShift: $e');
      return ApiResponse(
        success: false,
        errorMessage: e.toString(),
      );
  ...
  Future<ApiResponse<bool>> deleteShift(String shiftId) async {
    ...
    debugPrint('SHIFT_REPO: Starting shift deletion with ID: $shiftId');
    ...
    try {
      ...
      if (shiftId.isEmpty) {
        ...
        debugPrint('SHIFT_REPO: Cannot delete shift: Empty shift ID');
        return ApiResponse(
          success: false,
      ...
      if (shiftId.contains('mock')) {
        ...
        debugPrint('SHIFT_REPO: Cannot delete mock shift with ID: $shiftId');
        return ApiResponse(
          success: false,
      ...
      debugPrint('SHIFT_REPO: Checking if shift has assigned employees before deletion...');
      try {
        ...
        if (employeesResponse.success && employeesResponse.data != null && employeesResponse.data!.isNotEmpty) {
          debugPrint('SHIFT_REPO: Found ${employeesResponse.data!.length} employees assigned to this shift');
          return ApiResponse(
            success: false,
        ...
        debugPrint('SHIFT_REPO: Error checking assigned employees: $e');
      ...
      debugPrint('SHIFT_REPO: Using URL for deletion: $url');
      
      // Make the API call
      debugPrint('SHIFT_REPO: Sending DELETE request to $url with auth token');
      final response = await _apiService.delete<bool>(
        url,
        token: token,
        fromJson: (json) {
          // For DELETE operations, we might not get a response body
          debugPrint('SHIFT_REPO: Delete response received: $json');
      ...
      if (response.success) {
        ...
        debugPrint('SHIFT_REPO: Shift deleted successfully!');
      } else {
        developer.log('Failed to delete shift: ${response.errorMessage}');
        debugPrint('SHIFT_REPO: Failed to delete shift: ${response.errorMessage}');
      ...
      debugPrint('SHIFT_REPO: Exception in deleteShift: $e');
      return ApiResponse(
        success: false,
        errorMessage: e.toString(),
      );
  ...
  Future<void> checkAllowedMethods() async {
    try {
      ...
      if (token == null) {
        debugPrint('CHECK_METHODS: No auth token available');
        return;
      }
      
      final petrolPumpId = JwtDecoder.getClaim<String>(token, 'petrolPumpId');
      if (petrolPumpId == null) {
        debugPrint('CHECK_METHODS: No petrol pump ID available');
        return;
      }
      
      debugPrint('CHECK_METHODS: Running check for shift endpoints...');
      ...
      for (final url in urls) {
        debugPrint('CHECK_METHODS: Checking endpoint: $url');

        try {
          // Try different methods to see which ones work
          debugPrint('CHECK_METHODS: Trying GET for $url');
          final getResponse = await http.get(
            Uri.parse(url),
            headers: {'Authorization': 'Bearer $token'},
          );
          debugPrint('CHECK_METHODS: GET status for $url: ${getResponse.statusCode}');

          // Optional: Try POST with empty data
          debugPrint('CHECK_METHODS: Trying POST for $url');
          ...
          debugPrint('CHECK_METHODS: POST status for $url: ${postResponse.statusCode}');

          // Try PUT with empty data
          debugPrint('CHECK_METHODS: Trying PUT for $url');
          ...
          debugPrint('CHECK_METHODS: PUT status for $url: ${putResponse.statusCode}');

          // Summary
          debugPrint('CHECK_METHODS: Summary for $url:');
          debugPrint('  GET: ${getResponse.statusCode == 200 ? "✅" : "❌"} (${getResponse.statusCode})');
          debugPrint('  POST: ${postResponse.statusCode == 200 || postResponse.statusCode == 201 ? "✅" : "❌"} (${postResponse.statusCode})');
          debugPrint('  PUT: ${putResponse.statusCode == 200 || putResponse.statusCode == 201 ? "✅" : "❌"} (${putResponse.statusCode})');

        } catch (e) {
          debugPrint('CHECK_METHODS: Error checking $url: $e');
        }
      }
      ...
      debugPrint('CHECK_METHODS: Specifically testing correct endpoint: $correctUrl');
      ...
      try {
        ...
        debugPrint('CHECK_METHODS: POST to correct endpoint status: ${response.statusCode}');
        debugPrint('CHECK_METHODS: POST to correct endpoint body: ${response.body}');
      } catch (e) {
        debugPrint('CHECK_METHODS: Error testing correct endpoint: $e');
      }
    } catch (e) {
      debugPrint('CHECK_METHODS: Exception: $e');
    }
  }
  ...
  Future<ApiResponse<List<Employee>>> getEmployeeDetailsForShift(String shiftId) async {
    ...
    debugPrint('SHIFT_REPO: Getting employee details for shift ID: $shiftId');
    ...
    try {
      ...
      if (token == null) {
        debugPrint('SHIFT_REPO: No auth token found for getting employee details');
        return ApiResponse(
          success: false,
          errorMessage: 'You are not logged in. Please login to continue.',
        );
      ...
      final response = await _apiService.get<List<Employee>>(
        ...
        token: token,
        fromJson: (json) {
          debugPrint('SHIFT_REPO: Employee details response: $json');
          List<dynamic> employeeList = [];
          
          if (json is Map && json.containsKey('data') && json['data'] is List) {
            employeeList = json['data'] as List;
            debugPrint('SHIFT_REPO: Found employees in data field, count: ${employeeList.length}');
          } else if (json is List) {
            employeeList = json;
            debugPrint('SHIFT_REPO: Got direct list of employees, count: ${employeeList.length}');
          } else {
            debugPrint('SHIFT_REPO: Unexpected response format: $json');
            return <Employee>[];
          }
          ...
          try {
            employees = employeeList.map((item) {
              debugPrint('SHIFT_REPO: Processing employee item: $item');
              ...
              if (item['employeeName'] != null) {
                debugPrint('SHIFT_REPO: Found employeeName: ${item['employeeName']}');
              ...
              debugPrint('SHIFT_REPO: Mapped employee data: $mappedItem');
              return Employee.fromJson(mappedItem);
            }).toList();
            
            debugPrint('SHIFT_REPO: Successfully processed ${employees.length} employees for the shift');
          } catch (e) {
            debugPrint('SHIFT_REPO: Error converting JSON to Employee objects: $e');
            return <Employee>[];
          }
      ...
      if (response.success) {
        ...
        debugPrint('SHIFT_REPO: Successfully retrieved ${response.data?.length ?? 0} employees for the shift');
      } else {
        developer.log('Failed to retrieve employee details for shift: ${response.errorMessage}');
        debugPrint('SHIFT_REPO: Failed to retrieve employee details for shift: ${response.errorMessage}');
      }
      
      return response;
    } catch (e) {
      developer.log('Exception in getEmployeeDetailsForShift: $e');
      debugPrint('SHIFT_REPO: Exception in getEmployeeDetailsForShift: $e');
      return ApiResponse(
        success: false,
        errorMessage: e.toString(),
      );
